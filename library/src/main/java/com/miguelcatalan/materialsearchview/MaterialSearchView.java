...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener {
    ...
    public void updateSuggesstions(String[] suggestions) {
        if (suggestions != null && suggestions.length > 0 && mAdapter != null) {
            mTintView.setVisibility(VISIBLE);
            ((SearchAdapter)mAdapter).setSuggestions(suggestions);
            //((SearchAdapter)mAdapter).notifyDataSetChanged();

            setOnItemClickListener(new AdapterView.OnItemClickListener() {
                @Override
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    setQuery((String) mAdapter.getItem(position), true);
                    closeSearch();
                }
            });
        } else {
